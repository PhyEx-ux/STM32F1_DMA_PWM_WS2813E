...
void HDD_EXTI_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4; //使用PA4引脚为接受数据引脚 （其实还没测试过这个参数能不能用
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_2MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPD;
	GPIO_Init(GPIOA, &GPIO_InitStructure);

	GPIO_EXTILineConfig(GPIO_PortSourceGPIOA, GPIO_PinSource4); //配置GPIO为中断引脚
...
{
	if (EXTI_GetITStatus(EXTI_Line4) != RESET)  //检测是否触发对应的中断
	{
		HDD_Path_Flag = 1;
	}
